...
const loggEvent = (event: string, ekstraData?: object) => {
  logging.logEvent(event, { ...ekstraData, appName: 'dp-faktasider' });
};

export const loggError = (msg: string, ekstraData?: object) => {
  loggEvent('Error', { ...ekstraData, msg });
};

export const loggSidevisning = (side: string, ekstraData?: object) => {
  loggEvent('Side vist', { ...ekstraData, side });
};
